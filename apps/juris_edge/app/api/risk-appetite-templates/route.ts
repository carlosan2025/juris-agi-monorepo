...
import { IndustryProfile, Prisma } from '@prisma/client';
import {
 ...
 */
function transformFileTemplates(templates: RiskAppetiteTemplate[]) {
...
/**
 * GET /api/risk-appetite-templates
 * Returns risk appetite templates filtered by industry and/or company.
...
export async function GET(request: Request) {
  try {
    ...
    const conditions: Prisma.RiskAppetiteTemplateWhereInput = {};
    ...
    if (industry) {
      ...
      const fileTemplates = industryParam
    ...
    let templates = await prisma.riskAppetiteTemplate.findMany({
      where: conditions,
      orderBy: [
        { isDefault: 'desc' },
        { name: 'asc' },
    ...
    console.error('Failed to fetch risk appetite templates from database, using file fallback:', error);
...
/**
 * POST /api/risk-appetite-templates
 * Creates a new company-specific risk appetite template.
...
export async function POST(request: Request) {
  try {
    ...
    if (!name || !description || !industry || !riskData) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: name, description, industry, riskData' },
        { status: 400 }
      );
    }
    ...
    const template = await prisma.riskAppetiteTemplate.create({
      data: {
        name,
        description,
    ...
    console.error('Failed to create risk appetite template:', error);
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json(
      { success: false, error: 'Failed to create risk appetite template', details: errorMessage },
      { status: 500 }
    );
  }
}
