...
import {
  ...
  getAllGovernanceTemplates as getAllFileTemplates,
 ...
 */
function transformFileTemplates(templates: GovernanceTemplate[]) {
 ...
 * Returns governance templates filtered by industry and/or company.
...
export async function GET(request: Request) {
  try {
    ...
    const conditions: Prisma.GovernanceTemplateWhereInput = {};
    ...
    let templates = await prisma.governanceTemplate.findMany({
      where: conditions,
      orderBy: [
        { isDefault: 'desc' },
        { name: 'asc' },
    ...
    console.error('Failed to fetch governance templates from database, using file fallback:', error);
 ...
 * Creates a new company-specific governance template.
...
export async function POST(request: Request) {
  try {
    ...
    if (!name || !description || !industry || !governanceData) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: name, description, industry, governanceData' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to create governance template', details: errorMessage },
      { status: 500 }
    );
  }
}
