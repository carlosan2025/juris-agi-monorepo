 ...
 *
 * Authorized users:
 * - OWNER or ORG_ADMIN (company-level admins)
 * - Portfolio members with MAKER access level
 */
export async function POST(
  ...
  try {
    ...
    // Check if user has MAKER access on this portfolio
    ...
    // Must be either company admin or portfolio maker to submit
    if (!isCompanyAdmin && !isMaker) {
      return NextResponse.json(
        { success: false, error: 'Only administrators or makers can submit baselines for approval' },
        { status: 403 }
      );
    }
