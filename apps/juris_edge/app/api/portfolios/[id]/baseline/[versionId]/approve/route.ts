 ...
 * - Portfolio members with CHECKER access level
 */
export async function POST(
  ...
  try {
    ...
    // Check if user has CHECKER access on this portfolio
    ...
    // Must be either company admin or portfolio checker to approve
    if (!isCompanyAdmin && !isChecker) {
      return NextResponse.json(
        { success: false, error: 'Only administrators or checkers can approve baselines' },
        { status: 403 }
      );
    }
