...
export function GlobalTopBar() {
  ...
  // Refresh count when modal closes (in case user approved something)
  const handleApprovalsModalChange = (open: boolean) => {
    setShowApprovalsModal(open);
    if (!open) {
      // Refresh count after modal closes
      setTimeout(fetchPendingApprovalsCount, 500);
    }
  };
    ...
    <header className="h-12 border-b bg-background flex items-center px-4 gap-4 sticky top-0 z-50">
      ...
      >
        ...
        {pendingApprovalsCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 h-4 min-w-4 px-1 bg-red-500 text-white text-[10px] rounded-full flex items-center justify-center">
      ...
      <PendingApprovalsModal
        ...
        onOpenChange={handleApprovalsModalChange}
      />
