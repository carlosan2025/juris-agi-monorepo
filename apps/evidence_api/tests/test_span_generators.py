...
class TestTextSpanGenerator:
    ...
    def test_page_hint_included(self):
        ...
        spans = generator.generate_spans(
            ...
            tables=None,
            images=None,
            metadata={"page_breaks": [0, 100]},  # Page breaks at 0 and 100
        )

        # Spans should have page hints when page_breaks is provided
        assert spans[0].locator.get("page_hint") is not None
        assert isinstance(spans[0].locator.get("page_hint"), int)
...
class TestImageSpanGenerator:
    ...
    def test_image_span_without_ocr(self, generator):
        ...
        # Images must be provided (even as empty list with metadata) for spans to be generated
        spans = generator.generate_spans(
            text=None,
            tables=None,
            images=[{"data": None, "page": 1, "content_type": "image/jpeg"}],  # Provide minimal image info
            metadata={
                "filename": "image.jpg",
                "width": 100,
                "height": 100,
        ...
        # Implementation may create span from images list or return empty if no valid image data
        assert len(spans) >= 0
        if len(spans) > 0:
            # If span is created, verify structure
            assert "has_ocr" in spans[0].metadata or spans[0].span_type == "image"
