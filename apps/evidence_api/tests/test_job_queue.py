...
class TestJobQueueMocked:
    ...
    def test_enqueue_creates_job_record(self, mock_db_session, mock_redis):
        ...
        # Mock the queue functions inside the connection module where they're imported from
        with patch("evidence_repository.queue.connection.get_queue") as mock_get_queue:
    ...
    def test_enqueue_selects_queue_by_priority(self, mock_db_session, mock_redis):
        ...
        # Patch IS_SERVERLESS to False so Redis is used
        with patch("evidence_repository.queue.job_queue.IS_SERVERLESS", False):
            with patch("evidence_repository.queue.connection.get_high_priority_queue") as mock_high:
                with patch("evidence_repository.queue.connection.get_low_priority_queue") as mock_low:
                    with patch("evidence_repository.queue.connection.get_queue") as mock_normal:
                        # Setup mock queues
