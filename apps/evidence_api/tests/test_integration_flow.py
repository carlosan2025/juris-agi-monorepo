...
class TestUploadDocument:
    ...
    Note: The upload endpoint returns 202 Accepted with a job_id since processing
    is now async. The response contains {document_id, version_id, job_id, message}.
    """

    @pytest.mark.asyncio
    async def test_upload_text_document(self, client, sample_text_content):
        """Should upload a text document successfully and return job tracking info."""
...
class TestProjectManagement:
    ...
    async def test_attach_document_to_project(self, client, sample_text_content):
        ...
        doc_id = str(doc_response.json()["document_id"])
...
class TestDocumentExtraction:
    ...
    async def test_get_document_with_extracted_text(self, client, sample_text_content):
        """Should return document details after upload."""
        ...
        # API returns document fields directly, not nested versions
        assert "id" in data
...
class TestSearch:
    ...
    async def test_semantic_search_endpoint(self, client):
        ...
        # 200 if OpenAI configured, 503 if embeddings unavailable
    ...
    async def test_project_scoped_search(self, client):
        ...
        # Search within project (search router is at /search, so endpoint is /search/projects/{id})
        response = await client.post(
            f"/api/v1/search/projects/{proj_id}",
            json={
...
class TestFullPipeline:
    ...
    async def test_complete_document_flow(self, client, sample_text_content):
        ...
        assert doc_response.status_code == 202  # Accepted for async processing
        doc_data = doc_response.json()
        doc_id = str(doc_data["document_id"])
        version_id = str(doc_data["version_id"])
        ...
        # Step 5: Search within project (search router is at /search)
    ...
    async def test_document_version_tracking(self, client, sample_text_content):
        ...
        doc_id = str(v1_response.json()["document_id"])
        v1_id = str(v1_response.json()["version_id"])
...
class TestAsyncJobProcessing:
    ...
    async def test_async_upload_returns_job_id(self, client, sample_text_content):
        ...
        # Should return job info, or 500 if Redis is unavailable
    ...
    async def test_get_job_status(self, client):
        ...
        # Should return 404 for non-existent job, or 500 if Redis is unavailable
