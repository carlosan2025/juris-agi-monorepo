...
@pytest.fixture
def sample_version_number() -> int:
    """Generate a sample version number."""
    return 1
...
class TestGeneratePathKey:
    ...
    def test_generates_correct_path_key(self, local_storage, sample_document_id, sample_version_number):
        """Should generate path key in expected format."""
        path_key = local_storage.generate_path_key(sample_document_id, sample_version_number, "document.pdf")
        assert path_key == f"documents/{sample_document_id}/v{sample_version_number}/document.pdf"

    def test_sanitizes_filename(self, local_storage, sample_document_id, sample_version_number):
        """Should sanitize special characters in filename."""
        path_key1 = local_storage.generate_path_key(sample_document_id, sample_version_number, "my file (1).pdf")
        path_key2 = local_storage.generate_path_key(sample_document_id, sample_version_number, "myfile1.pdf")
        # Both should be sanitized to safe characters
        assert "/" not in path_key1.split("/")[-1].replace("/", "")  # Last part should not have additional slashes
